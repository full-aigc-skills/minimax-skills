...
description: 面向音乐人、视频创作者和社交媒体剪辑者，用于制作带歌词贴字的 AI MV 或情绪短片。用户提供音乐、歌词、参考图、角色、字体方向、情绪意图或发布平台。Skill 会分析节拍与人声时序，区分人物、场景和文字参考，设计随节奏变化的空间字幕，拆解长视频为可衔接镜头，审查提示词并路由 H3 等视频生成工具。最终输出 MV 概念、分镜提示词、歌词文字方案和拼接建议。适用于风格化音乐视觉和动态字幕 MV，不适用于普通字幕校对、照搬已有 IP 或完全手工后期剪辑。
...
本技能是同名 MiniMax-H3 Hub 技能的 mmx-cli 改编版：创意方法论保持不变，
Hub 画布工具替换为以下可移植等价物。

| Hub 工具 | mmx-cli / 可移植等价物 |
|---|---|
| `hub_generate_image` | `mmx image generate --prompt "..." --out <路径>` |
| `hub_generate_video` | `mmx video generate --prompt "..." --out <路径>`（Hailuo；时长/模型参数见 `mmx video generate --help`） |
| `hub_generate_audio` / `hub_synthesize_speech` | `mmx speech synthesize --text "..." --out <路径.mp3>` |
| `hub_generate_music` | `minimax-music-gen` 技能（MiniMax 音乐 API；mmx 无音乐命令） |
| `hub_video_edit` | ffmpeg 合成（concat / xfade / 音频混音）——确定性、离线 |
| `hub_analyse_media` | `ffprobe` + `mmx vision describe <文件>` |
| `hub_image_search` | 优先用户提供素材；公开参考可用 `mmx search query "..."` |
| `hub_canvas_get_node` / `hub_canvas_group_recent_outputs` | 剧集工作目录台账：`assets-manifest.md`、`outputs/`、`decisions.md` |
| choice cards | 在对话中呈现编号选项，选择记录到 `decisions.md` |

前置条件：`npm install -g mmx-cli` 并 `mmx auth login --api-key sk-xxx`；合成与
检查需要 ffmpeg/ffprobe 在 PATH。

# 音乐美学MV

## 用途

当用户需要创建、修改、审查或生成音乐视频提示词、情绪短片提示词时使用本 Skill，尤其是音乐、歌词、贴字、参考图、节奏、人物表演和镜头语言需要统一设计的任务。本 Skill 将第三方 MV prompt 规则适配为 mmx-cli 可执行流程：关键创意决策需要确认，锁定 prompt 写入剧集目录的 markdown 文件，媒体生成通过 mmx CLI 完成。
...
- 锁定 prompt 和修订版本必须写入剧集目录的 markdown 文件（如 complete-mv-prompt.md）。
- 角色卡、场景卡、文字卡、视频片段、BGM 和最终合成通过 mmx CLI（image/video/speech）与 ffmpeg 完成。
- 始终用 --out / --out-dir 把输出显式写到剧集输出目录。
